...
There is a second named pattern worth a sentence: **validating against something you authored.** Third catch of the same disease — the schema-parity check that compared the fixture against a manifest transcribed from the same docs (ADR-003a, fixed by capturing live), and the order-history catalog path whose only demo would have run against history we seeded ourselves (ADR-030, dropped). The test is one question: *who wrote the thing this check passes against?* If the answer is "we did," the check is theatre until the reference comes from outside.
